                ...
                {step === 1 && (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-gray-400 text-xs md:text-sm">Current Location <span className="text-red-500">*</span></label>
                                <input required type="text" name="location" value={formData.location} onChange={handleChange} className="w-full bg-dark-bg/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-neon-blue transition-colors" placeholder="City, State" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-gray-400 text-xs md:text-sm">Company <span className="text-red-500">*</span></label>
                                <input required type="text" name="company" value={formData.company} onChange={handleChange} className="w-full bg-dark-bg/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-neon-blue transition-colors" placeholder="Acme Corp" />
                            </div>
                        </div>

                        <div className="flex justify-end pt-4">
                            <button type="button" onClick={handleNext} disabled={!formData.fullName || !formData.email || !formData.mobile || !formData.location || !formData.company} className="bg-white disabled:opacity-50 disabled:cursor-not-allowed text-black font-bold py-2 md:py-3 px-6 md:px-8 rounded-lg hover:bg-gray-200 transition-colors flex items-center text-sm md:text-base cursor-target">
                ...
                {step === 2 && (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                            <div className="space-y-2">
                                <label className="text-gray-400 text-xs md:text-sm">Budget Range <span className="text-red-500">*</span></label>
                                <select required name="budget" value={formData.budget} onChange={handleChange} className="w-full bg-dark-bg/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-neon-blue transition-colors appearance-none cursor-pointer">
                                    <option value="">Select Budget</option>
                                    {budgets.map(b => b !== "Please Select" && <option key={b} value={b} className="bg-dark-bg text-gray-300">{b}</option>)}
                                </select>
                            </div>
                            <div className="space-y-2">
                                <label className="text-gray-400 text-xs md:text-sm">Timeline <span className="text-red-500">*</span></label>
                                <select required name="timeline" value={formData.timeline} onChange={handleChange} className="w-full bg-dark-bg/50 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-neon-blue transition-colors appearance-none cursor-pointer">
                        ...
                        <div className="flex justify-between pt-8 border-t border-white/10">
                            ...
                            <button
                                type="submit"
                                disabled={isSubmitting || !formData.description || !formData.budget || !formData.timeline}
